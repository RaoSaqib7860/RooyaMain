...
class EditPost extends StatefulWidget {
  ...
  final RooyaPostModel? postModel;

  const EditPost({this.fromEvent = false, this.event_id = '0', this.postModel});
...
class _EditPostState extends State<EditPost> {
  ...
  getPath() async {
    ...
    for (var i in widget.postModel!.attachment!) {
        ...
        if ('${appDocDir.path}/$name'.contains('mp4')) {
          if (!controller.listOfSelectedfiles
            ...
            controller.listOfSelectedfiles
                .add({'video': '${appDocDir.path}/$name'});
          }
        } else {
          if (!controller.listOfSelectedfiles
              .contains('${appDocDir.path}/$name')) {
            controller.listOfSelectedfiles
                .add({'image': '${appDocDir.path}/$name'});
          }
        }
      }
    }
  ...
  Future download2({String? url, String? savePath}) async {
    try {
      ...
      if (file.path.contains('mp4')) {
        if (!controller.listOfSelectedfiles.contains('${file.path}')) {
          controller.listOfSelectedfiles.add({'video': '${file.path}'});
        }
      } else {
        if (!controller.listOfSelectedfiles.contains('${file.path}')) {
          controller.listOfSelectedfiles.add({'image': '${file.path}'});
        }
      }
      await raf.close();
    } catch (e) {
                      ...
                      children: [
                        ...
                        InkWell(
                          onTap: () async {
                            ...
                            snackBarSuccess('Update post successfully');
  ...
  Future<void> createPost(List files) async {
    ...
    final response = await http.post(Uri.parse('${baseUrl}rePost$code'),
